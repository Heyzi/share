...
class GitLabScanner:
    ...
    def scan_group(self, group_id: int) -> Tuple[List[Dict], Dict[str, int]]:
        initial_count = 0
        projects_info = []
        summary = {'total_projects': 0, 'total_repo_size': 0, 'total_storage_size': 0,
        ...
        try:
            def scan_recursive(group_id):
                ...
                for project in group.projects.list(all=True):
                    initial_count += 1
                    if info := self.get_project_info(project.id, project, project.namespace['full_path']):
                        projects_info.append(info)
                        summary['total_projects'] += 1
                        summary['total_repo_size'] += info['repository_size']
                        summary['total_storage_size'] += info['storage_size']
                        summary['total_artifacts_size'] += info['artifacts_size']
                        if info['has_ci']: summary['projects_with_ci'] += 1
...
template = """
<!DOCTYPE html><html><head><meta charset="utf-8"><title>GitLab Projects Report</title>
...
thead input{width:100%;padding:3px;box-sizing:border-box;margin-top:5px}
</style></head><body class="container-fluid">
<div class="summary-card"><h2>Projects Report - {{ summary.scan_timestamp }}</h2>
...
<div class="table-responsive"><table class="table table-striped table-bordered" id="projectsTable">
<thead><tr><th>#</th><th>Namespace</th><th>Project</th><th>URL</th><th>Branch</th>
<th>Repository Size</th><th>Storage Size</th><th>Artifacts Size</th><th>CI Config</th>
<th>Last Commit</th><th>Last Pipeline</th></tr></thead><tbody>
{% for project in projects %}<tr><td>{{ loop.index }}</td><td>{{ project.namespace }}</td>
...
<td>{{ 'Yes' if project.has_ci else 'No' }}</td><td class="date-cell">{{ project.last_commit }}</td>
<td class="date-cell">{{ project.last_pipeline }}</td></tr>{% endfor %}</tbody></table></div>
<script>
$(document).ready(function(){$('#projectsTable').DataTable({paging:false,ordering:true,info:true,
fixedHeader:true,orderCellsTop:true,search:{regex:true,smart:false},
